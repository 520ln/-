...
!function ($) {
    ...
    function showlist(sid, num) {
        $.get("http://localhost/js2002/xiangmu/php/alldata.php",
            function (data) {
                ...
                $.each(data, function (index, value) {
                    if (sid === value.sid) {
                        ...
                        $clonebox.find('.cart_shop_img img').attr('src', value. url)
                        $clonebox.find('.cart_shop_img img').attr('sid', value.sid)
                        $clonebox.find('.cart_shop_text p').html(value.title)
                        $clonebox.find('.cart_shop_pirce span').html("￥" + value.price)
                        $clonebox.find('.cart_shop_num input').val(num)
                        $clonebox.find('.cart_shop_total span').html((value.price * num).toFixed(2))
    ...
    function total() {
        ...
        $('.cart_main_shop_num_price i').html($piece+"件")
    }
    ...
    $('.cart_shop_remove .cart_shop_remove_remove').on("click", function () {
        ...
        $jxgw.on('click', function(){
            location.href='http://localhost/js2002/xiangmu/src/list.html'
        });
}(jQuery)